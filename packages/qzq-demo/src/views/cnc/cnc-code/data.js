// CNC 代码注解表数据
const cncData = [
  { code: 'G00', description: '快速定位（快速移动）' },
  { code: 'G01', description: '直线插补 / 直线切削' },
  { code: 'G02', description: '顺时针圆弧（CW）' },
  { code: 'G03', description: '逆时针圆弧（CCW）' },
  { code: 'G04', description: '暂停（Dwell）' },
  { code: 'G05.1', description: '预读控制' },
  { code: 'G09', description: '停于精确的位置' },
  { code: 'G10', description: '可编程数据输入' },
  { code: 'G11', description: '可编程数据输入方式取消' },
  { code: 'G15', description: '极坐标指令取消' },
  { code: 'G16', description: '极坐标指令' },
  { code: 'G17', description: '选择XY平面 / XY面赋值' },
  { code: 'G18', description: '选择ZX平面 / XZ面赋值' },
  { code: 'G19', description: '选择YZ平面 / YZ面赋值' },
  { code: 'G20', description: '英制输入' },
  { code: 'G21', description: '公制输入' },
  { code: 'G22', description: '内部行程限位有效' },
  { code: 'G23', description: '内部行程限位无效' },
  { code: 'G27', description: '检查参考点返回' },
  { code: 'G28', description: '参考点复位 / 参考点返回' },
  { code: 'G29', description: '从参考点返回' },
  { code: 'G30', description: '回到第二参考点（换刀点）' },
  { code: 'G32', description: '切螺纹' },
  { code: 'G37', description: '自动刀具长度测量' },
  { code: 'G40', description: '取消半径补偿 / 取消刀尖半径偏置' },
  { code: 'G41', description: '刀具半径左补偿 / 刀尖半径偏置（左侧）' },
  { code: 'G42', description: '刀具半径右补偿 / 刀尖半径偏置（右侧）' },
  { code: 'G43', description: '刀具长度正补偿 / 刀具长度+方向偏移' },
  { code: 'G44', description: '刀具长度负补偿 / 刀具长度-方向偏移' },
  { code: 'G49', description: '刀具长度补偿取消 / 取消刀具长度偏移' },
  { code: 'G50', description: '取消比例缩放 / 修改工件坐标设置主轴最大RPM' },
  { code: 'G50.1', description: '镜像功能取消' },
  { code: 'G51', description: '比例缩放' },
  { code: 'G51.1', description: '镜像功能开启' },
  { code: 'G52', description: '局部坐标系（局部坐标偏移）' },
  { code: 'G53', description: '机床坐标系选择' },
  { code: 'G54', description: '工件坐标系1选择' },
  { code: 'G54.1', description: '扩展坐标系P1-P48' },
  { code: 'G55', description: '工件坐标系2选择' },
  { code: 'G56', description: '工件坐标系3选择' },
  { code: 'G57', description: '工件坐标系4选择' },
  { code: 'G58', description: '工件坐标系5选择' },
  { code: 'G59', description: '工件坐标系6选择' },
  { code: 'G65', description: '宏程序调用' },
  { code: 'G66', description: '宏程序模态调用' },
  { code: 'G67', description: '宏程序模态调用取消' },
  { code: 'G68', description: '坐标系旋转' },
  { code: 'G69', description: '坐标系旋转取消' },
  { code: 'G70', description: '精加工循环' },
  { code: 'G71', description: '内外径粗切循环' },
  { code: 'G72', description: '台阶粗切循环' },
  { code: 'G73', description: '高速深孔钻削循环' },
  { code: 'G74', description: '左螺旋切削循环 / 左旋螺纹攻丝' },
  { code: 'G76', description: '精镗孔循环' },
  { code: 'G80', description: '取消固定循环' },
  { code: 'G81', description: '中心钻循环' },
  { code: 'G82', description: '钻孔固定循环底部暂停 / 反镗孔循环' },
  { code: 'G83', description: '深孔钻削循环' },
  { code: 'G84', description: '右螺旋切削循环 / 右旋螺纹攻丝' },
  { code: 'G85', description: '镗孔循环 / 粗镗孔' },
  { code: 'G86', description: '镗孔循环 / 精镗孔' },
  { code: 'G87', description: '反向镗孔循环' },
  { code: 'G88', description: '镗孔循环' },
  { code: 'G89', description: '镗孔循环' },
  { code: 'G90', description: '使用绝对值命令编程' },
  { code: 'G91', description: '使用增量值命令编程' },
  { code: 'G92', description: '设置工件坐标系' },
  { code: 'G94', description: '每分钟进给' },
  { code: 'G95', description: '每转进给' },
  { code: 'G98', description: '固定循环返回初始点 / 返回起始点' },
  { code: 'G99', description: '固定循环返回R点' },
  { code: 'M00', description: '程序暂停（强制暂停）' },
  { code: 'M01', description: '程序选择暂停 / 选择停止' },
  { code: 'M02', description: '程序结束' },
  { code: 'M03', description: '主轴正转（CW）' },
  { code: 'M04', description: '主轴反转（CCW）' },
  { code: 'M05', description: '主轴停止' },
  { code: 'M06', description: '换刀指令' },
  { code: 'M07', description: '吹气' },
  { code: 'M08', description: '切削液开' },
  { code: 'M09', description: '切削液关' },
  { code: 'M30', description: '程序结束并返回程序头' },
  { code: 'M98', description: '子程序调用' },
  { code: 'M99', description: '子程序结束并返回子程序头' },
  { code: 'F', description: '进给功能或者螺距' },
  { code: 'S', description: '每分钟转速 / 主轴转速' },
  { code: 'T', description: '刀具号' },
  { code: 'D', description: '半径补偿号' },
  { code: 'H', description: '长度补偿号' },
  { code: 'P', description: '调用子程序号或者暂停时间' },
  { code: 'L', description: '循环次数' },
  { code: 'Q', description: '钻孔参数' },
  { code: 'O', description: '程序名' },
  { code: 'R', description: '钻孔起始平面' },
  { code: 'N', description: '程序段号/行号' },
  { code: 'XYZ-ABC', description: '机床工作轴' },
]

// 数控26个英文代表含义数据
const code2Data = [
  { code: 'A', description: '绕X轴旋转' },
  { code: 'B', description: '绕Y轴旋转' },
  { code: 'C', description: '绕Z轴旋转' },
  { code: 'D', description: '补偿号刀偏半径补偿指令' },
  { code: 'E', description: '第2进给功能' },
  { code: 'F', description: '进给速度指令' },
  { code: 'G', description: '指令动作方式' },
  { code: 'H', description: '补偿号的指定' },
  { code: 'I', description: '圆弧中芯X轴坐标' },
  { code: 'J', description: '圆弧中芯Y轴坐标' },
  { code: 'K', description: '圆弧中芯Z轴坐标' },
  { code: 'L', description: '固定循环/子程序重复次数' },
  { code: 'M', description: '辅助功能机床开关指令' },
  { code: 'N', description: '顺序号顺序段序号' },
  { code: 'O', description: '子程序顺序号指定' },
  { code: 'P', description: '暂停/程序号' },
  { code: 'Q', description: '固定循环中的定距' },
  { code: 'R', description: '坐标字圆弧半径的指定' },
  { code: 'S', description: '主轴功能主轴转速指令' },
  { code: 'T', description: '刀偏功能刀偏编号指令' },
  { code: 'U', description: '与X轴平行增量坐标值' },
  { code: 'V', description: '与Y轴平行增量坐标值' },
  { code: 'W', description: '与Z轴平行增量坐标值' },
  { code: 'X', description: '主轴坐标值' },
  { code: 'Y', description: '轴坐标值' },
  { code: 'Z', description: '轴坐标值' },
]
// 常用刀具的刃长数据
const code3Data = [
  { spec: 'PRO-4F001-RN', diameter: 'φ1', length: '3', handle_diameter: '4', handle_length: '50' },
  { spec: 'PRO-4F002-RN', diameter: 'φ1.5', length: '4', handle_diameter: '4', handle_length: '50' },
  { spec: 'PRO-4F003-RN', diameter: 'φ2', length: '6', handle_diameter: '4', handle_length: '50' },
  { spec: 'PRO-4F004-RN', diameter: 'φ2.5', length: '8', handle_diameter: '4', handle_length: '50' },
  { spec: 'PRO-4F005-RN', diameter: 'φ3', length: '8', handle_diameter: '3', handle_length: '50' },
  { spec: 'PRO-4F006-RN', diameter: 'φ3', length: '8', handle_diameter: '4', handle_length: '50' },
  { spec: 'PRO-4F007-RN', diameter: 'φ3.5', length: '8', handle_diameter: '4', handle_length: '50' },
  { spec: 'PRO-4F008-RN', diameter: 'φ4', length: '10', handle_diameter: '4', handle_length: '50' },
  { spec: 'PRO-4F009-RN', diameter: 'φ5', length: '13', handle_diameter: '5', handle_length: '50' },
  { spec: 'PRO-4F010-RN', diameter: 'φ5', length: '13', handle_diameter: '6', handle_length: '50' },
  { spec: 'PRO-4F011-RN', diameter: 'φ6', length: '15', handle_diameter: '6', handle_length: '50' },
  { spec: 'PRO-4F012-RN', diameter: 'φ8', length: '20', handle_diameter: '8', handle_length: '60' },
  { spec: 'PRO-4F013-RN', diameter: 'φ3', length: '12', handle_diameter: '3', handle_length: '75' },
  { spec: 'PRO-4F014-RN', diameter: 'φ4', length: '15', handle_diameter: '4', handle_length: '75' },
  { spec: 'PRO-4F015-RN', diameter: 'φ5', length: '20', handle_diameter: '5', handle_length: '75' },
  { spec: 'PRO-4F016-RN', diameter: 'φ6', length: '20', handle_diameter: '6', handle_length: '75' },
  { spec: 'PRO-4F017-RN', diameter: 'φ8', length: '25', handle_diameter: '8', handle_length: '75' },
  { spec: 'PRO-4F018-RN', diameter: 'φ10', length: '25', handle_diameter: '10', handle_length: '75' },
  { spec: 'PRO-4F019-RN', diameter: 'φ12', length: '30', handle_diameter: '12', handle_length: '75' },
  { spec: 'PRO-4F020-RN', diameter: 'φ14', length: '32', handle_diameter: '14', handle_length: '80' },
  { spec: 'PRO-4F021-RN', diameter: 'φ3', length: '12', handle_diameter: '3', handle_length: '100' },
  { spec: 'PRO-4F022-RN', diameter: 'φ4', length: '16', handle_diameter: '4', handle_length: '100' },
  { spec: 'PRO-4F023-RN', diameter: 'φ5', length: '20', handle_diameter: '5', handle_length: '100' },
  { spec: 'PRO-4F024-RN', diameter: 'φ6', length: '25', handle_diameter: '6', handle_length: '100' },
  { spec: 'PRO-4F025-RN', diameter: 'φ8', length: '35', handle_diameter: '8', handle_length: '100' },
  { spec: 'PRO-4F026-RN', diameter: 'φ10', length: '40', handle_diameter: '10', handle_length: '100' },
  { spec: 'PRO-4F027-RN', diameter: 'φ12', length: '45', handle_diameter: '12', handle_length: '100' },
  { spec: 'PRO-4F028-RN', diameter: 'φ14', length: '45', handle_diameter: '14', handle_length: '100' },
  { spec: 'PRO-4F029-RN', diameter: 'φ16', length: '45', handle_diameter: '16', handle_length: '100' },
  { spec: 'PRO-4F030-RN', diameter: 'φ18', length: '45', handle_diameter: '18', handle_length: '100' },
  { spec: 'PRO-4F031-RN', diameter: 'φ20', length: '45', handle_diameter: '20', handle_length: '100' },
  { spec: 'PRO-4F032-RN', diameter: 'φ25', length: '45', handle_diameter: '25', handle_length: '100' },
  { spec: 'PRO-4F033-RN', diameter: 'φ6', length: '25', handle_diameter: '6', handle_length: '150' },
  { spec: 'PRO-4F034-RN', diameter: 'φ8', length: '35', handle_diameter: '8', handle_length: '150' },
  { spec: 'PRO-4F035-RN', diameter: 'φ10', length: '45', handle_diameter: '10', handle_length: '150' },
  { spec: 'PRO-4F036-RN', diameter: 'φ12', length: '45', handle_diameter: '12', handle_length: '150' },
  { spec: 'PRO-4F037-RN', diameter: 'φ14', length: '45', handle_diameter: '14', handle_length: '150' },
  { spec: 'PRO-4F038-RN', diameter: 'φ16', length: '50', handle_diameter: '16', handle_length: '150' },
  { spec: 'PRO-4F039-RN', diameter: 'φ18', length: '50', handle_diameter: '18', handle_length: '150' },
  { spec: 'PRO-4F040-RN', diameter: 'φ20', length: '55', handle_diameter: '20', handle_length: '150' },
  { spec: 'PRO-4F041-RN', diameter: 'φ25', length: '60', handle_diameter: '25', handle_length: '150' },
]

const code4Data = [
  {
    coarse_spec: 'M1.0x0.25',
    coarse_std: '0.75',
    coarse_max: '0.785',
    coarse_min: '0.729',
    fine_spec: 'M1.0x0.2',
    fine_std: '0.80',
    fine_max: '0.821',
    fine_min: '0.783',
  },
  {
    coarse_spec: 'M1.1x0.25',
    coarse_std: '0.85',
    coarse_max: '0.885',
    coarse_min: '0.829',
    fine_spec: 'M1.1x0.2',
    fine_std: '0.90',
    fine_max: '0.921',
    fine_min: '0.883',
  },
  {
    coarse_spec: 'M1.2x0.25',
    coarse_std: '0.95',
    coarse_max: '0.985',
    coarse_min: '0.929',
    fine_spec: 'M1.2x0.2',
    fine_std: '1.00',
    fine_max: '1.021',
    fine_min: '0.983',
  },
  {
    coarse_spec: 'M1.4x0.3',
    coarse_std: '1.10',
    coarse_max: '1.142',
    coarse_min: '1.075',
    fine_spec: 'M1.4x0.2',
    fine_std: '1.20',
    fine_max: '1.221',
    fine_min: '1.183',
  },
  {
    coarse_spec: 'M1.6x0.35',
    coarse_std: '1.25',
    coarse_max: '1.321',
    coarse_min: '1.221',
    fine_spec: 'M1.6x0.2',
    fine_std: '1.40',
    fine_max: '1.421',
    fine_min: '1.383',
  },
  {
    coarse_spec: 'M1.7x0.35',
    coarse_std: '1.35',
    coarse_max: '1.421',
    coarse_min: '1.321',
    fine_spec: 'M1.7x0.2',
    fine_std: '1.50',
    fine_max: '1.521',
    fine_min: '1.483',
  },
  {
    coarse_spec: 'M1.8x0.35',
    coarse_std: '1.45',
    coarse_max: '1.521',
    coarse_min: '1.421',
    fine_spec: 'M1.8x0.2',
    fine_std: '1.60',
    fine_max: '1.621',
    fine_min: '1.583',
  },
  {
    coarse_spec: 'M2.0x0.4',
    coarse_std: '1.60',
    coarse_max: '1.679',
    coarse_min: '1.576',
    fine_spec: 'M2.0x0.25',
    fine_std: '1.75',
    fine_max: '1.785',
    fine_min: '1.729',
  },
  {
    coarse_spec: 'M2.2x0.45',
    coarse_std: '1.75',
    coarse_max: '1.838',
    coarse_min: '1.713',
    fine_spec: 'M2.2x0.25',
    fine_std: '1.95',
    fine_max: '1.985',
    fine_min: '1.929',
  },
  {
    coarse_spec: 'M2.3x0.4',
    coarse_std: '1.90',
    coarse_max: '1.979',
    coarse_min: '1.867',
    fine_spec: 'M2.3x0.25',
    fine_std: '2.05',
    fine_max: '2.085',
    fine_min: '2.029',
  },
  {
    coarse_spec: 'M2.5x0.45',
    coarse_std: '2.10',
    coarse_max: '2.138',
    coarse_min: '2.013',
    fine_spec: 'M2.5x0.35',
    fine_std: '2.20',
    fine_max: '2.221',
    fine_min: '2.121',
  },
  {
    coarse_spec: 'M2.6x0.45',
    coarse_std: '2.20',
    coarse_max: '2.238',
    coarse_min: '2.113',
    fine_spec: 'M2.6x0.35',
    fine_std: '2.20',
    fine_max: '2.321',
    fine_min: '2.221',
  },
  {
    coarse_spec: 'M3.0x0.5',
    coarse_std: '2.50',
    coarse_max: '2.599',
    coarse_min: '2.459',
    fine_spec: 'M3.0x0.35',
    fine_std: '2.70',
    fine_max: '2.721',
    fine_min: '2.621',
  },
  {
    coarse_spec: 'M3.5x0.6',
    coarse_std: '2.90',
    coarse_max: '3.010',
    coarse_min: '2.850',
    fine_spec: 'M3.5x0.35',
    fine_std: '3.20',
    fine_max: '3.221',
    fine_min: '3.121',
  },
  {
    coarse_spec: 'M4.0x0.7',
    coarse_std: '3.30',
    coarse_max: '3.422',
    coarse_min: '3.242',
    fine_spec: 'M4.0x0.5',
    fine_std: '3.50',
    fine_max: '3.599',
    fine_min: '3.459',
  },
  {
    coarse_spec: 'M4.5x0.75',
    coarse_std: '3.80',
    coarse_max: '3.878',
    coarse_min: '3.688',
    fine_spec: 'M4.5x0.5',
    fine_std: '4.00',
    fine_max: '4.099',
    fine_min: '3.959',
  },
  {
    coarse_spec: 'M5.0x0.8',
    coarse_std: '4.20',
    coarse_max: '4.334',
    coarse_min: '4.134',
    fine_spec: 'M5.0x0.5',
    fine_std: '4.50',
    fine_max: '4.599',
    fine_min: '4.459',
  },
  {
    coarse_spec: 'M6.0x1.0',
    coarse_std: '5.00',
    coarse_max: '5.153',
    coarse_min: '4.917',
    fine_spec: 'M5.5x0.5',
    fine_std: '5.00',
    fine_max: '5.099',
    fine_min: '4.959',
  },
  {
    coarse_spec: 'M7.0x1.0',
    coarse_std: '6.00',
    coarse_max: '6.153',
    coarse_min: '5.917',
    fine_spec: 'M6.0x0.75',
    fine_std: '5.30',
    fine_max: '5.378',
    fine_min: '5.188',
  },
  {
    coarse_spec: 'M8.0x1.25',
    coarse_std: '6.80',
    coarse_max: '6.912',
    coarse_min: '6.647',
    fine_spec: 'M6.0x0.5',
    fine_std: '5.50',
    fine_max: '5.599',
    fine_min: '5.459',
  },
  {
    coarse_spec: 'M9.0x1.25',
    coarse_std: '7.80',
    coarse_max: '7.912',
    coarse_min: '7.647',
    fine_spec: 'M7.0x0.75',
    fine_std: '6.30',
    fine_max: '6.378',
    fine_min: '6.188',
  },
  {
    coarse_spec: 'M10x1.5',
    coarse_std: '8.50',
    coarse_max: '8.676',
    coarse_min: '8.376',
    fine_spec: 'M7.0x0.5',
    fine_std: '6.50',
    fine_max: '6.599',
    fine_min: '6.459',
  },
  {
    coarse_spec: 'M11x1.5',
    coarse_std: '9.50',
    coarse_max: '9.676',
    coarse_min: '9.376',
    fine_spec: 'M8.0x1.0',
    fine_std: '7.00',
    fine_max: '7.153',
    fine_min: '6.917',
  },
  {
    coarse_spec: 'M12x1.75',
    coarse_std: '10.30',
    coarse_max: '10.441',
    coarse_min: '10.106',
    fine_spec: 'M8.0x0.75',
    fine_std: '7.30',
    fine_max: '7.378',
    fine_min: '7.188',
  },
  {
    coarse_spec: 'M14x2.0',
    coarse_std: '12.00',
    coarse_max: '12.210',
    coarse_min: '11.835',
    fine_spec: 'M8.0x0.5',
    fine_std: '7.50',
    fine_max: '7.599',
    fine_min: '7.459',
  },
  {
    coarse_spec: 'M16x2.0',
    coarse_std: '14.00',
    coarse_max: '14.210',
    coarse_min: '13.835',
    fine_spec: 'M9.0x1.0',
    fine_std: '8.00',
    fine_max: '8.153',
    fine_min: '7.917',
  },
  {
    coarse_spec: 'M18x2.5',
    coarse_std: '15.50',
    coarse_max: '15.744',
    coarse_min: '15.294',
    fine_spec: 'M9.0x0.75',
    fine_std: '8.30',
    fine_max: '8.378',
    fine_min: '8.188',
  },
  {
    coarse_spec: 'M20x2.5',
    coarse_std: '17.50',
    coarse_max: '17.744',
    coarse_min: '17.294',
    fine_spec: 'M10x1.25',
    fine_std: '8.80',
    fine_max: '8.912',
    fine_min: '8.647',
  },
  {
    coarse_spec: 'M22x2.5',
    coarse_std: '19.50',
    coarse_max: '19.744',
    coarse_min: '19.294',
    fine_spec: 'M10x1.0',
    fine_std: '9.00',
    fine_max: '9.153',
    fine_min: '8.917',
  },
  {
    coarse_spec: 'M24x3.0',
    coarse_std: '21.00',
    coarse_max: '21.252',
    coarse_min: '20.752',
    fine_spec: 'M10x0.75',
    fine_std: '9.30',
    fine_max: '9.378',
    fine_min: '9.188',
  },
  {
    coarse_spec: 'M27x3.0',
    coarse_std: '24.00',
    coarse_max: '24.252',
    coarse_min: '23.752',
    fine_spec: 'M10x0.5',
    fine_std: '9.50',
    fine_max: '9.599',
    fine_min: '9.459',
  },
  {
    coarse_spec: 'M30x3.5',
    coarse_std: '26.50',
    coarse_max: '26.771',
    coarse_min: '26.211',
    fine_spec: 'M11x1.0',
    fine_std: '10.00',
    fine_max: '10.153',
    fine_min: '9.917',
  },
  {
    coarse_spec: 'M33x3.5',
    coarse_std: '29.50',
    coarse_max: '29.771',
    coarse_min: '29.211',
    fine_spec: 'M11x0.75',
    fine_std: '10.30',
    fine_max: '10.378',
    fine_min: '10.188',
  },
  {
    coarse_spec: 'M36x4.0',
    coarse_std: '32.00',
    coarse_max: '32.270',
    coarse_min: '31.670',
    fine_spec: 'M12x1.5',
    fine_std: '10.50',
    fine_max: '10.676',
    fine_min: '10.376',
  },
  {
    coarse_spec: 'M39x4.0',
    coarse_std: '35.00',
    coarse_max: '35.270',
    coarse_min: '34.670',
    fine_spec: 'M12x1.25',
    fine_std: '10.80',
    fine_max: '10.912',
    fine_min: '10.647',
  },
]

const columns = [
  { prop: 'code', label: '代码', width: 250 },
  { prop: 'description', label: '注解' },
]

// 创建一个上下文，获取指定目录下的所有图片文件
const images = import.meta.glob('./images/*.{jpg,jpeg,png,gif,svg}', { eager: true })

// 处理图片对象，提取文件名作为 key，并按第一个小数点前的数字排序
const imageMap = {}

// 将图片对象转换为数组并按文件名中的数字排序
const sortedEntries = Object.entries(images)
  .map(([path, module]) => {
    // 提取文件名（去掉路径和扩展名）
    const fileName = path.match(/\/([^/]+)\.\w+$/)[1]
    return { fileName, module: module.default }
  })
  .sort((a, b) => {
    // 提取第一个小数点前面的数字进行比较
    const numA = parseInt(a.fileName.split('.')[0]) || 0
    const numB = parseInt(b.fileName.split('.')[0]) || 0
    return numA - numB
  })

// 将排序后的结果存入对象
for (const item of sortedEntries) {
  imageMap[item.fileName] = item.module
}
export const tabs = [
  {
    name: 'code1',
    label: '代码注解表',
    data: cncData,
    columns,
  },
  {
    name: 'code2',
    label: '数控26个英文代表含义',
    data: code2Data,
    columns,
  },
  {
    name: 'code33',
    label: '常用刀具的刃长',
    data: code3Data,
    columns: [
      { prop: 'spec', label: '型号', width: 250 },
      { prop: 'diameter', label: '刀刃直径D(φ)' },
      { prop: 'length', label: '刀刃长L(mm)' },
      { prop: 'handle_diameter', label: '刀柄直接D(φ)' },
      { prop: 'handle_length', label: '全长L(mm)' },
    ],
  },
  {
    name: 'code3',
    label: '图例及刀具',
    data: imageMap,
  },
  {
    name: 'code4',
    label: '螺丝攻牙钻孔对照表',
    data: code4Data,
  },
]
